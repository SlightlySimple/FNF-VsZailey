...
class OptionsMenu extends FlxSubState
{
	...
	override function create()
	{
		options = [
			...
			new OptionCata(345, 40, "Appearance", [
				...
				new StepManiaAdvancedOption("Customize how quantization coloring works."),
				new AccuracyOption("Display accuracy information on the info bar."),
				new SongPositionOption("Show the song's current position as a scrolling bar."),
				new Colour("The color behind icons now fit with their theme."),
				new NPSDisplayOption("Shows your current Notes Per Second on the info bar."),
				new RainbowFPSOption("Make the FPS Counter flicker through rainbow colors."),
				new CpuStrums("Toggle the CPU's strumline lighting up when it hits a note."),
				new ColorIconsOption("Recolor the icons on the health bar to accurately resemble the characters."),
			]),
			...
			new OptionCata(-1, 125, "Editing Judgements", [
				new SickMSOption("How many milliseconds are in the SICK hit window (Press R to reset)"),
				new GoodMsOption("How many milliseconds are in the GOOD hit window (Press R to reset)"),
				new BadMsOption("How many milliseconds are in the BAD hit window (Press R to reset)"),
				new ShitMsOption("How many milliseconds are in the SHIT hit window (Press R to reset)")
			], true),
			new OptionCata(-1, 125, "Editing Quantization Settings", [
				new RotateNotesOption("Rotate notes to the proper direction (Recommended to disable if using a circle noteskin)"),
				new QuantColorOption("The color quarter notes use when quantization colors are enabled.",0),
				new QuantColorOption("The color eighth notes use when quantization colors are enabled.",1),
				new QuantColorOption("The color twelfth notes use when quantization colors are enabled.",2),
				new QuantColorOption("The color sixteenth notes use when quantization colors are enabled.",3),
				new QuantColorOption("The color twenty fourth notes use when quantization colors are enabled.",4),
				new QuantColorOption("The color thirty second notes use when quantization colors are enabled.",5)
			], true)
		];
	...
	public function switchCat(cat:OptionCata, checkForOutOfBounds:Bool = true)
	{
		try
		{
			...
			if (selectedCatIndex > options.length - 4 && checkForOutOfBounds)
				selectedCatIndex = 0;
