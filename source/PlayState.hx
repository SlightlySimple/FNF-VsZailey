...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (dialogue.length > 0)
		{
			...
			doof.cameras = [camHUD];
			doof.finishThing = doofStartCountdown;
		}
		...
		if (dialogue.length > 0 && !PlayState.seenDialogue)
			startDialogue(doof);
		else
		{
		...
		if (generatedMusic)
		{
			if (songStarted && !endingSong)
			{
				...
				if (unspawnNotes.length == 0 && notes.length == 0 && FlxG.sound.music.time > FlxG.sound.music.length - 100)
				{
					...
					endingSong = true;
					healthGraphInfo.push( [Conductor.songPosition, health] );
					if (Paths.doesTextAssetExist(Paths.txt('data/songs/${PlayState.SONG.songId}/dialogueEnd')))
						endSongDialogue();
					else
						endSong();
				}
	...
	function endSongDialogue():Void
	{
		...
		var dialogueEnd = CoolUtil.coolTextFile(Paths.txt('data/songs/${PlayState.SONG.songId}/dialogueEnd'));

		var doof = new DialogueBox(false, dialogueEnd);
		doof.cameras = [camHUD];
